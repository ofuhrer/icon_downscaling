...
slot, and one CPU slot. Retryable scheduler interruptions have no fixed
attempt limit (`max_model_attempts=0`); application or validation failures
still block immediately. It uses `preemptible` for HICAR and `pp-short` for
