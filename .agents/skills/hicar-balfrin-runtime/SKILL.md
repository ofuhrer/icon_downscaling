...
`scancel --batch` signals the Slurm batch shell, not an arbitrary Python child.
Any pre-emptible runner that must continue with validation after its guarded
model command returns must source `scripts/preemption_signal_forwarding.sh`
and call `run_with_preemption_signal_forwarding`; do not merely invoke the
Python guard as a foreground child. The cheap TERM/KILL probe uses this same
helper so it exercises the real batch-shell boundary before HICAR compute is
spent.
