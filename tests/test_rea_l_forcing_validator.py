...
def files(root: Path, invalid_hfl: bool = False) -> tuple[Path, Path, Path]:
    ...
    with netCDF4.Dataset(static, "w") as dataset:
        ...
        dataset.createDimension("half_level", 3)
    ...
    with netCDF4.Dataset(forcing, "w") as dataset:
        ...
        hfl = np.array([50.0, 210.0 if invalid_hfl else valid_top])[:, None, None] * np.ones(
            (1, 2, 2)
        )
...
def test_sst_unsupported_water_mask_must_match_count(tmp_path: Path) -> None:
    forcing, static, boundary = files(tmp_path)
    with netCDF4.Dataset(forcing, "a") as dataset:
        dataset["SST_unsupported_water_mask"][0, 1] = 1
        dataset["SST_nearest_same_surface_candidate_distance_km"][0, 1] = 5.0
    with netCDF4.Dataset(boundary, "a") as dataset:
        dataset.initial_condition_sha256 = sha256(forcing)
    result = subprocess.run(
        [
    ...
    assert result.returncode != 0
    assert "unsupported-water mask disagrees with its count" in result.stderr
