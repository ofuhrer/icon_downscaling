...
SPEC = importlib.util.spec_from_file_location("sst_unsupported_support", SCRIPT)
...
def make_inputs(tmp_path: Path, *, unsupported_on_land: bool = False, no_unsupported: bool = False):
    ...
    unsupported_mask = np.zeros((ny, nx), dtype=np.int8)
    unsupported_distance = np.full((ny, nx), np.nan, dtype=np.float64)
    if not no_unsupported:
        unsupported_cells = ((0, 0, 1.0), (2, 2, 3.0), (4, 2, 5.0))
        for row, column, distance in unsupported_cells:
            unsupported_mask[row, column] = 1
            unsupported_distance[row, column] = distance
    if unsupported_on_land:
        unsupported_mask[0, 7] = 1
        unsupported_distance[0, 7] = 2.0
    ...
    with netCDF4.Dataset(forcing, "w") as dataset:
        ...
        dataset.createVariable("SST_unsupported_water_mask", "i1", ("y_1", "x_1"))[:] = (
            unsupported_mask
        )
        ...
        distance[:] = unsupported_distance
        dataset.sst_water_cell_count = len(water_cells)
        dataset.sst_water_unsupported_count = unsupported_count
        dataset.sst_maximum_nearest_same_surface_candidate_distance_km = (
            float(np.nanmax(unsupported_distance)) if unsupported_count else 0.0
        )
...
def test_reports_area_components_distances_and_unique_stations(tmp_path: Path) -> None:
    ...
    assert summary["unsupported_water_cell_count"] == 3
    assert summary["unsupported_water_area_km2"] == 1.5
    assert summary["unsupported_fraction_of_water"] == 3 / 8
    distances = summary["nearest_same_surface_candidate_distance_km"]
    ...
    assert components["unsupported_affected_component_count"] == 2
    by_area = components["components"]
    assert [component["water_cell_count"] for component in by_area] == [4, 3, 1]
    assert [component["unsupported_cell_count"] for component in by_area] == [1, 2, 0]
    assert by_area[0]["unsupported_fraction_of_component_water"] == 0.25
    ...
    assert by_key["EXA:1"]["nearest_unsupported_cell_distance_km"] == pytest.approx(0.0)
    assert (
        by_key["EXA:1"]["nearest_unsupported_cell"]["nearest_same_surface_candidate_distance_km"]
        == 1.0
    )
    assert by_key["EXB:2"]["nearest_unsupported_cell_distance_km"] > 1.0


def test_no_unsupported_has_null_distribution_and_station_distance(tmp_path: Path) -> None:
    static, forcing, observations, _ = make_inputs(tmp_path, no_unsupported=True)

    payload = MODULE.build_report(forcing, static, observations)

    assert payload["summary"]["nearest_same_surface_candidate_distance_km"] == {
        "count": 0,
        "maximum": None,
        "p50": None,
        "p90": None,
    ...
    assert payload["water_components"]["unsupported_affected_component_count"] == 0
    assert all(
        site["nearest_unsupported_cell_distance_km"] is None
        for site in payload["stations"]["sites"]
    )


def test_rejects_unsupported_mask_on_static_land(tmp_path: Path) -> None:
    static, forcing, observations, report = make_inputs(tmp_path, unsupported_on_land=True)
...
def test_component_retention_keeps_largest_and_material_components() -> None:
    ...
    unsupported_counts = np.zeros(26, dtype=np.int64)
    water_counts[1:] = 1

    selected, reasons = MODULE.select_component_ids(
        water_counts, unsupported_counts, cell_area_km2=0.04
    )
    ...
    assert "unsupported_area_ge_0.2_km2" in reasons[25]
