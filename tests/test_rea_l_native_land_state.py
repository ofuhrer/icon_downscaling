...
def inventory():
    surface = [field("SKT", "K"), field("W_SNOW", "kg m**-2"), field("RHO_SNOW", "kg m-3")]
...
def test_native_surface_inventory_is_exact() -> None:
    ...
    with pytest.raises(ValueError, match="instantaneous"):
        bad = [surface[0], field("W_SNOW", "kg m-2", step_type="accum"), surface[2]]
...
def test_extpar_and_snow_ranges_are_checked() -> None:
    ...
    surface = {"SKT": np.array([280.0, 281.0]), "W_SNOW": np.array([0.0, 10.0]),
